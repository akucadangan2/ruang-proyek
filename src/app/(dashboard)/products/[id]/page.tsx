...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/products/${id}`)
      ...
      .then((res) => {
        ...
        setFormFields(res.data.order_form_fields ?? []);
        setBumps(res.data.order_bumps ?? []);
      });

    fetch("/api/products")
      .then((res) => res.json())
      .then(({ data }) => setAllProducts((data ?? []).filter((p: Product) => p.id !== id)));
  }, [id]);
  ...
  if (!product) return <p className="p-6 text-[13px] text-ink-soft">Memuat...</p>;
        ...
        <div className="space-y-4">
          ...
          <OrderBumpForm
            ...
            onAdd={(bumpProductId) =>
              setBumps((prev) => [
                ...prev,
                { id: crypto.randomUUID(), product_id: id, bump_product_id: bumpProductId, auto_checked: false, sort_order: prev.length },
              ])
            }
            onRemove={(bumpId) => setBumps((prev) => prev.filter((b) => b.id !== bumpId))}
            onToggleAutoChecked={(bumpId, value) =>
              setBumps((prev) => prev.map((b) => (b.id === bumpId ? { ...b, auto_checked: value } : b)))
            }
          />
